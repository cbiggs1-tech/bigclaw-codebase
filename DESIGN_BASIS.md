...
| Version | July 22, 2026 (four-sleeve cutover + Commando radar/doctrine; shadow OpenRouter/Grok Judge still experimental — not production) |
| Author | Curtis Biggs |
| Hostname | BigClaw |
...
5. [Portfolio Management Philosophy](#5-portfolio-management-philosophy)
   ...
   - 5.3 LLM Discretionary / Commando
   - 5.4 Four-Sleeve Live Book (July 2026)
...
BigClaw historically managed up to nine independent paper portfolios. **As of the July 21–22 2026 four-sleeve cutover, four portfolios are live (`is_active=1`); the rest are retained in the DB as inactive history** (see §5.4). Style fidelity remains non-negotiable for the rule-based books. LLM-Commando is the event-driven discretionary stock book (see §5.3).

### 5.1 Portfolio Definitions

| # | Portfolio | Investment Style | Live? | Modeled After |
|---|-----------|-----------------|-------|---------------|
| 1 | Value Picks | Quality Value Investing | **Inactive** (Jul 2026 cutover) | Buffett / Graham |
| 2 | Innovation Fund | Disruptive Innovation | **Active** | Cathie Wood / ARK |
| 3 | Growth Value | Growth at Reasonable Price (GARP) | **Inactive** | Peter Lynch |
| 4 | Income Dividends | Income / Dividend Growth | **Inactive** | Dividend Aristocrats |
| 5 | Momentum Growth | CANSLIM Momentum | **Active** | William O'Neil |
| 6 | Nuclear Renaissance | Nuclear Energy / Domain Expertise | **Inactive** | Thematic Structural |
| 7 | AI Defense & Autonomous | AI Defense / Autonomous Systems | **Active** | Pentagon Spending Theme |
| 8 | LLM-ETF Focus | LLM-Driven ETF-tilted dialectic | **Inactive** | None — pure model judgment |
| 9 | LLM-Commando | LLM-Driven Single-Stock dialectic + radar | **Active** | None — pure model judgment |
| — | Treasury Reserve | Cash bucket (historical) | **Inactive** (retired May 2026) | — |
...
- **LLM-ETF Focus** (formerly named LLM Discretionary, renamed June 10 2026) — original variant, trades whatever the dialectic settles on with no preference. Observed behavior is sector-rotation via SPDR ETFs as the LLM defaults to the most data-rich investable objects (XLK, XLF, XLE, etc.) and rotates between them on macro/news shifts. Curtis observed this and decided to preserve the behavior intact as a real-world test of LLM-driven sector rotation. **Inverse ETFs / short capability (July 2 2026).** ETF Focus's universe was long-only (sector SPDRs + long factor/macro ETFs), so a bearish regime read could only be expressed as cash or defensive longs — at the time it sat ~86% cash after hawkish-Fed commentary, forecasting a decline it had no way to profit from. Added `INVERSE_ETFS = [SH, PSQ, DOG, RWM]` (-1x S&P/Nasdaq-100/Dow/Russell) to the universe, the market snapshot, and a rendered candidate block. These are ordinary LONG buys that rise when the market falls, so they need no shorting/margin infrastructure and fit the long-only execution + ETF-only mandate. The prompt framing (per Curtis) gates them tightly: cash is the correct call in a genuine UNKNOWN (no directional edge); an inverse ETF is for a CONFIRMED bearish edge only — the SAME VIX 22+ / credit-breadth-deterioration bar used to size down, PLUS a macro driver pushing the tape lower — and explicitly NOT in a low/normal-VIX tape (high-teens VIX is a long-or-cash regime, not a short). Deliberately -1x only; leveraged -2x/-3x inverse are excluded as bad-quadrant volatility-decay risk on the 1-day-to-1-week horizon. Shorts are sized with a target/stop and covered when the down-move plays out — never married. (VIX was low when this shipped; it is a capability for the next down-regime, not a directive to short.)

- **LLM-Commando** (new June 10 2026) — variant with **strict stock-preference language** in all three agent prompts (Bull, Bear, Judge). ETFs are explicitly forbidden except for hedging. Candidate universe expanded to include ~60 curated liquid names from `portfolio_universes.json` (the BigClaw rule-based portfolios' shared universe) so per-ticker Benzinga news flows into the context for individual-stock theses. Dry-run verified: Judge picked AMAT (analyst PT raises) and APA (geopolitical energy thesis) — no ETFs.
...
**Curated LESSONS block + extension/RSI in the compete view (July 17 2026; lessons 5–6 July 22 2026).** An evidence-based diagnostic of BigClaw's own trades (415 real buys, 2026-04 → 07) found the single strongest predictor of poor forward return is how *extended* a name is at entry — buys far above their 200-day MA or overbought (RSI>70) mean-revert, the most-extended quintile returning ~−8% vs SPY. A shared `scripts/llm_lessons.py` module now injects a curated **LESSONS LEARNED** block into Commando's data feed every cycle (Bull, Bear, and Judge all read it): (1) extension is risk, not confirmation; (2) buying strength is not an edge; (3) opportunity cost is the exit; (4) a live catalyst is the edge, price alone is a bandwagon; **(5) threatened tariffs / political headlines are not enacted policy** — prefer real reported numbers; only in-force policy (or company already booking the cost) can veto a clean earnings thesis (07-21 lesson: Judge skipped a real beat+raise on Canada-tariff *threat* framing); **(6) day-2 chase after the run is not edge** — rehash of yesterday's earnings after a multi-day pop with no new fact = stand down. These are **information, not hard rules** — the dialectic still decides. Commando flags overbought/extended names in the compete view with `⚠extended`. Soft LLM governance vs hard entry-gate for rule books (§6.5b).

**Commando event radar + price-is-news (July 21–22 2026).** Waiting only for 09:00 / 11:30 / 14:30 CT dialectic sessions was too slow for day-trader style. `scripts/llm_comando_radar.py` polls **every 2 minutes** during RTH (`*/2 8-15 * * 1-5`), reads new Alpaca news since a cursor, and runs a cheap Sonnet **GO** decision without full Bull→Bear→Judge when there is something to act on. Shared daily fire budget: `MAX_EVENT_FIRES_PER_DAY = 100` in `llm_comando_news.py` (~$0.02–0.03/fire). **price_is_news (holdings fade):** multi-day stock loss with *no* fresh headline is itself an event — if SPY session is not down and the sector ETF session is not down, and the name lags its sector over ~3d, radar wakes research and defaults to **sell half** (system clamps to 50%; residual keeps trailing stops). Market/sector weakness alone is never a sell reason. Cooldown ~20h per ticker; fade scan throttled to ~15 minutes.

**Cold-trader doctrine (July 22 2026).** Operator laws in Bull/Bear/Judge + radar NORTH STAR:
- **REAL > THEATER** — earnings / formal guidance / signed deals beat threatened tariffs; only **ENACTED** (in force) policy can veto a print.
- **NO CHASE** — same-session reaction to a live print can still have room; day-2+ rehash after multi-day run with no new fact → stand down.
- **COLD EDGE (not passivity)** — constraints remove human FOMO / threat-fear; they do **not** mean hug cash or trade less. Selective aggression: early on real facts, absent when late and crowded.

**OpenRouter LIVE (graduated 2026-07-22):** Commando full dialectic + radar + watcher run on OpenRouter — Sonnet Bull/Bear + **Grok-4.5 Judge**. Dual shadow path **OFF** (former shadow *is* live). Requires `OPENROUTER_API_KEY`. Direct Anthropic no longer required for Commando trading; Slack bot / Claude Code may still use Anthropic if configured.

**Hold scorecard - the third learning signal (June 26 2026).** The learning loop now covers all three decision types. Buy/sell calls are scored by the reconciler (closed positions vs `exit_thesis`); no-buy calls by the refusal scorecard (above); and now HOLD calls by a hold scorecard. A hold is a real decision under the conviction-exit doctrine, but it had been invisible - each cycle the LLM "held" by deciding zero sells, with no measure of what the hold cost. Commando now tracks each open position's high-water-mark unrealized % (`data/llm_comando_position_peaks.json`) and renders the GIVE-BACK (peak minus current) into context each cycle, e.g. "PODD held since 06-23: now +8.4%, peaked +12.4% - GIVEN BACK 4.0 pts by holding; would you BUY it here today, or are you riding a winner back down?" It also flags a hold that has gone outright negative. This makes the classic failure - riding a winner back down, or sitting in a loser hoping for recovery - a visible, learnable cost instead of an implicit non-decision. Per Curtis: a hold that loses over time should teach, the same as a bad buy or a costly refusal.

**The alpha drive - installing a gas pedal, not just releasing brakes (June 29 2026).** First live run with all the above showed the limit: Commando read the refusal scorecard ("74% of names I passed rose"), agreed it was a failure mode, and *still* held 0 trades - rationalizing the evidence rather than acting. Per Curtis: every fix so far had only *removed brakes* (VIX, cash-isn't-free, room-to-run, scorecards); none installed a *drive*. The LLM's implicit objective was still "don't lose," which cash satisfies perfectly, so it defaulted to cash and explained it away. The fix changes what Commando optimizes for: (1) the Judge objective is now explicitly to GENERATE ALPHA - beat SPY and the rule-based bots - and a money-market parking posture that risks nothing is named the surest failure ("a thing whose best move is cash is not an investor"); the peer-return block is framed as a scoreboard it is losing. (2) Cash is reframed from safety to FORFEIT, with the burden of proof flipped onto it - to hold cash the Judge must defend the specific claim "the names available to me are more likely to fall than rise," which on a risk-on day (SPY up, VIX normal, credit risk-on) is hard to defend. (3) The reflection field now forces a benchmark-relative self-assessment ("are you BEATING SPY and the bots? if you are trailing while in cash, your caution is the failure, not the edge") to counter the self-reinforcing "27 cycles, passivity is my edge" prior. (4) Persona: an alpha HUNTER, not a goalie, embarrassed to end a green day in cash, who hunts HARDER when the obvious news-makers are weak - and Curtis's framing: "cash is ammunition, not a fortress; deploy it to plunder the market's cash and grow the war chest." Guardrail preserved: every entry still requires a real, citable, still-playing-out catalyst with room to run - the drive is to find the genuine edge harder, never to manufacture one. First dry-run after the change: Commando went from 0 trades to deploying on a genuine catalyst (GOOGL DJIA-inclusion, mechanical multi-day passive flows) while explicitly noting it was losing to the bots. **Apples-to-apples peer scoreboard (July 7 2026).** The 'losing to the bots' framing relies on the peer-return block, which was comparing Commando's ~1-month track record (since its June 10 inception) against the rule-based peers' since-FEBRUARY cumulative - not comparable. `get_peer_returns` now baselines every peer to Commando's first `daily_snapshots` date and reports return over Commando's own window (same as Commando's cumulative), labeled as such. It materially changes the picture (e.g. Growth Value +3.9% since Jun 10 vs -2.8% since Feb; Momentum -6.6% vs -1.7%), so the scoreboard is honest rather than flattering or unfair. **Immediate post-trade trailing-stop init (same day).** The LLM cycles relied on the every-15-min stop_check cron to create trailing stops, so a position bought between runs was briefly without a `trailing_stops` row and got flagged UNPROTECTED by the refresh data-health-check (DOCN, bought 9:11, stop created 9:15). Both cycles now call `initialize_stops()` immediately after a filled buy (mirrors the rule-based trader's RULE 5.25). The position was never truly exposed - its `exit_conditions` stop is live from purchase via the 5-min mechanical exit - only the false alert is fixed.

**Commando momentum sourcing (added June 17 2026, REMOVED June 25 2026).** A `discover_momentum_leaders()` view briefly fed Commando relative-strength / breakout names from the rule-based screened universe (`signals.json`) alongside the news-makers, with a Judge "MOMENTUM SOURCING" mandate that treated a clean uptrend as a tradeable thesis even without a headline. **Reverted June 25 2026 per Curtis:** price momentum with no news behind it is bandwagon-chasing, not a thesis. Commando is back to news-driven discovery only — every entry must rest on a citable, still-playing-out catalyst; when the news set is thin, holding cash is the correct call rather than manufacturing a momentum trade. The VIX VOLATILITY REGIME mandate stays (it sizes aggression) but was retuned to drop the "ride the leaders" language.

**LLM-portfolio horizon reset (June 25 2026).** After a week of observation, two June-16-to-18 changes were reverted to restore the original experiment. (1) **ETF Focus back to SHORT-TERM.** The June-16 pivot had reframed ETF Focus as a multi-month thesis book (wide 25%/15% exits, thesis-break-only triggers, 30-min watcher, "hold through noise"). A longer horizon arguably suits an ETF book, but the experiment *learns faster* with short-term turnover, and the paired A/B only holds if both LLM portfolios share a horizon. ETF Focus is again a short-term, fast-turnover book: tight exits, intraday triggers, quick-profit/cut-loss doctrine, 5-min watcher — same horizon as Commando, differing only on instrument (ETFs vs single stocks). The good post-pivot additions were KEPT: the gap-analysis Judge mandate (retuned to a days-to-week lens), the unified cycle framing, the do-not-open-into-a-binary-event rule, and the macro-regime pack. (2) **Commando back to news-only discovery** (see the paragraph above).

**Judge model A/B — Fable 5 vs Opus 4.8 (concluded July 11 2026).** A live A/B ran Fable 5 as the Commando Judge (executing) with Opus 4.8 shadowing the identical Bull/Bear input. It concluded on **reliability, not decision quality**: Fable's long adaptive-thinking Judge requests get dropped / time out (`APITimeoutError`), and a timed-out Judge kills the entire cycle — no decisions, no trades. Record: one ~723s near-miss (July 9), then **two of three cycles FATAL on July 10** (morning + midday). Opus shadow ran ~110s with valid JSON every time. Reverted the live Judge to Opus 4.8 (Bull/Bear unchanged, Sonnet 4.6); the shadow A/B block was disabled. **Design lesson:** for a live trading Judge, reliability — completing within the cycle's time budget before the market close — *gates* decision quality, and must be tested under the real timeout, not just dry-runs. The Bull/Bear -> Opus promotion (held until the Judge was settled) is now unblocked as a separate future A/B.

**Every-session re-verification + time_exit retired (July 13 2026).** Commando now surfaces each holding's persisted ENTRY THESIS + original target + entry date into the Judge's compete view every session (the "WHY BOUGHT" anchor; the data was already stored in holdings.rationale/target_price, just not shown). Doctrine added: the entry thesis is a TIME-STAMPED HYPOTHESIS re-verified every session and classified STRENGTHENED / INTACT / WEAKENED / SPENT - never a standing justification. A few-hour catalyst cannot justify a multi-day hold; the position must be re-justified by CURRENT evidence, the entry thesis serving only as the yardstick for stronger-or-weaker-than-at-entry. Every stock earns its existence every session; holding period is an OUTPUT, not a target. The mechanical time_exit (clock-based forced sale) was RETIRED from evaluate_triggers (shared by watcher + reconciler) - it force-sold VOYA on a timer July 13 despite a fresh same-day KBW raise, contradicting the conviction-not-clock doctrine. stop_hit (risk rail) and target_hit (bank a planned gain between cycles, re-verified next cycle) are KEPT. Dry-run validated: the Judge re-verified all four holdings per-doctrine (MPC strengthening, VOYA intact, RJF 4-days-stale/watch-rotation, THC original-catalyst-stale-but-WF-raise-today-resets-the-clock). Pending: feed this-session mechanical exits into the Judge + a stop-loss reconciliation lens. **Evidence-grounded dialectic (July 14 2026).** Every SELL is now classified by REASONING not P&L: thesis_wrong (buy premise proved false - the learning; carries an exit_lesson), thesis_changed (rotation/banked-gain/regime-shift - NOT a negative), or thesis_played_out (predicted move happened - a win). A stop-out is thesis_wrong only if the premise was flawed. The classification is surfaced inline in the journal all three agents read. Bull mines past PLAYED-OUT sells for analogs to strengthen a buy; Bear mines past REASONING-ERROR sells for analogs to bolster an avoid/sell; the Judge classifies every sell (and unclassified mechanical stop-outs it reviews) and REFEREES - weighting track-record-backed arguments over assertions and flagging cherry-picking. Per Curtis: the learning lives in thesis_wrong; thesis_changed is never counted against anything.

**ETF Focus event-risk rule (added June 18 2026).** Before OPENING a new position, the ETF Focus Judge checks for a binary event (FOMC / CPI / jobs / earnings) in the next 1-2 days and does not establish fresh exposure into it — it waits for the event to clear. Existing positions are still held through events; only opening-into is barred. Added after a June 17 same-day whipsaw: ETF Focus bought XLF hours before an FOMC decision and had to sell it after the hawkish pivot (Commando's gap-analysis had caught the event risk; ETF noted the meeting but bought anyway).

**Agent token ceilings (raised June 17 2026).** The June 16 doctrine additions (the `gap_analysis` field, cycle-positioning / regime reasoning, the mandatory Bear priced-in test) made agent outputs longer and began truncating them — on June 17 the ETF Focus Judge overflowed a 4000-token cap and its JSON failed to parse, costing that cycle's deployment. Ceilings were raised for output room with no logic change: ETF Judge 4000→8000 (matching Commando), and the Bull/Bear debate caps 3000/4000→6000.
...
**Cost.** LLM-ETF Focus daily cycle ≈ $0.13 (three Sonnet 4.6 calls, smaller news context). LLM-Commando daily cycle ≈ $0.18 (same architecture but expanded ~60-ticker candidate universe inflates input tokens). Combined annual cost: ≈ $80 for ~252 trading days × 2 portfolios.

**Trade closure reconciler (added June 8 2026).** A second script `llm_portfolio_reconciler.py` runs daily at 14:50 CT (just before market close — moved from 15:30 CT on June 30 2026, see fix below) to verify each open position against its stated `exit_thesis`. Logic: in priority order, check stop > target > time_exit; if any trigger fires, submit the SELL via the canonical Alpaca + `record_trade` path. The Judge prompt now emits a structured `exit_conditions: {target_pct, stop_pct, time_exit_date}` alongside the prose `exit_thesis`; for legacy trades lacking the structured field, a small Sonnet 4.6 call (~$0.001) extracts the conditions on first encounter. Each closure writes an `outcome` record to `data/llm_outcomes.jsonl` capturing entry/exit prices, realized %, days held, which trigger fired, and whether the original prediction was correct. **This outcomes log is what makes recursive learning real** — without verified prediction outcomes, the daily Judge has no signal to learn from. Periodic synthesis (monthly, to be built ~July 1) will compress accumulated outcomes into a `llm_strategy_guide.md` that the daily prompt reads at top-of-context.

**Phantom-closure bug + timing fix (June 30 2026).** Two defects let a fired exit silently strand a position and corrupt the learning log. (1) The reconciler ran at 15:30 CT — 30 min AFTER the 16:00 ET close — and submitted a `TimeInForce.DAY` order, which cancels unfilled after hours (despite a stale code comment claiming orders "queue to next open"). So the sell filled 0 shares. (2) `main()` then wrote the closure `outcome` regardless of fill: with `filled_price` 0 it recorded `exit_price 0 -> realized_pct -100%`, while `record_trade` correctly refused the zero-share DB update — leaving the position still held but logged as closed. The next run then skipped it forever as an "already in outcomes but still held" anomaly. Commando's PODD (entry 06-23, target_hit 06-26) was orphaned this way for 4 days, riding +12% back down while the dashboard showed negative upside; ETF Focus's MTUM (06-15) left the same corrupt -100% record. Fix: (a) market-closed guard in `execute_close` — never submit a futile order when `clock.is_open` is false; (b) an integrity gate in `main()` — a closure outcome is recorded ONLY if the sell actually filled (`filled_qty > 0`); otherwise the position is left OPEN and retried, never fabricated as a -100% phantom; (c) both LLM reconciler crons moved to 14:50 CT so their sells fill during market hours. The two phantom records were purged (backed up) and PODD was exited cleanly (+5.19%, time_exit). FOLLOW-UP for the give-back concern: mechanical target/stop exits still only fire once/day at 14:50; moving them into the 5-min watcher would book gains at target-hit intraday ("take the money and run") instead of waiting for the daily sweep. SHIPPED same day (June 30 2026): `check_mechanical_exits()` now runs in BOTH watchers every 5 minutes during market hours, evaluating each holding's target/stop/time exit via the shared reconciler logic (`evaluate_triggers` + the new `build_outcome`/`compute_predicted_correctly` helpers) and selling through the watcher's clamped, short-safe path (`clamp_sell_to_long`) with the same fill-integrity gate (a closure is recorded only on a real fill). The reconciler stays as the after-close backstop; both write to the same outcomes log (deduped via origin_date+ticker) so neither double-closes, and each outcome now carries a `source` field (watcher|reconciler). This is the true fix for the PODD give-back: a target hit at 10am is booked at 10am, not at the 14:50 sweep. **Overnight/wartime risk weighting (July 8 2026).** The exit-by-conviction rule (which stopped Commando churning to cash) had an unintended edge: it labeled ALL trimming of a winner 'the churn error', so Commando held green names overnight into a war tape and gave the gains back (July 7: the Judge overrode the Bear's 'protect gains, arrive tomorrow with optionality' as churn, even after flagging the overnight risk itself; VRDN peaked +8.5%, exited +6.5%). Per Curtis: a BANKED gain has control/finality/loss-prevention; an unrealized gain is a loan the market can call back overnight; holding past the close buys nothing but gap exposure. Fix (pure doctrine, commit 45ab7d819): the conviction test gains a second half for a position on a real gain - weigh the overnight risk vs banking now and re-entering next session (sell-high/rebuy-low is NOT churn). The weight is modest in a calm tape and rises with war signals: military escalation in the news feed (Iran/Hormuz/tankers), oil trending up (USO), and VIX TREND/rate-of-change even under 22 (absolute VIX level dials INTRADAY aggression, TREND dials the OVERNIGHT-hold decision - separate dials). Give the Bear more weight in wartime. It is a rebalance of when-to-hold, NOT a retreat to cash (still deploy intraday, re-enter next session), so it preserves the alpha drive. The signals were already in the Judge's context; only the doctrine changed.

**Dashboard planned-actions made real — single source of truth (June 30 2026).** The website's "planned actions" panel was built by `build_planned_actions.py`, a *separate reimplementation* of the trader's logic that had drifted: it applied a blanket "sell anything held that is not in the top 10 by score" to every portfolio, ignoring (a) target-price discipline (Innovation Fund holds to its analyst price target, not the top-10 rule), (b) the style-gate filter on candidates, and (c) the per-portfolio universe. Result: it showed **12 planned sells when the trader would actually make 1** — e.g. 8 Innovation Fund names held under discipline, plus names pushed out of the top 10 by candidates the trader would never buy. Curtis: "the dashboard needs to reflect what's real, not a phantom plan." Fix: the trader's per-portfolio planning block was extracted into a module-level `plan_portfolio()` in `autonomous_trader.py` (returns to_sell/to_buy/target, applying the universe + style gate + target-price discipline — pure planning, no execution), and `build_planned_actions.py` now CALLS that function instead of re-deriving the plan. The dashboard panel is therefore exactly what the trader will do. Buys are still sized cash-aware for display. (Emergency Phase-1 safety sells, score <= -3, are rare and not separately projected.)
...
**Watcher execution-aware retry + fill-failure alert (added June 25 2026).** A decided intraday trade was being treated as done based on the LLM's *decision*, not on whether it *filled*. On June 23 2026, as the market sold off at the open, the Commando watcher's protective SELL of LRCX was submitted as a market order ~1 minute after the open, did not fill within the 30s window, and was canceled (`filled 0/15`) — yet the trigger was marked `consumed` and a fire counted, so the exit was silently abandoned and the position stayed exposed (the 09:00 deliberative cycle independently sold it 33 minutes later; books stayed reconciled — no phantom shares, no short). Fix, applied to **both** watchers: (1) a submitted-but-underfilled trade keeps its trigger in a new `retrying` status and is re-attempted on later polls — re-running the focused LLM call, which re-validates conviction and re-clamps to the live Alpaca long via `clamp_sell_to_long`, so a retry can never oversell or open a short — capped at `EXEC_RETRY_CAP=3` attempts, after which the trigger is `consumed` and flagged `exec_failed`; (2) any decided trade that does not fill now raises a loud ⚠️ Slack alert ("position STILL HELD, retrying / MANUAL ACTION NEEDED") instead of the previous buried `(0/N trades)` line. `cleanup_obsolete_triggers` was extended to also clear `retrying` triggers whose position has since been closed by another path. Auto-retry reliably covers price-stop exits (the condition persists and re-fires); news/time-trigger fill failures rely on the alert plus the next scheduled cycle. Note the ETF Focus watcher polls every 30 min vs Commando's 5 min, so its retry cadence is slower.

**Experimental purpose.** The rule-based sleeves are the control. Commando is the treatment (event-driven + dialectic). Direct comparison: does LLM judgment with structured self-feedback and radar beat hand-coded rules and SPY?

### 5.4 Four-Sleeve Live Book (July 21–22 2026)

**Goal:** concentrate labor and capital on books that can generate alpha with lower ops load; cut thematic/duplicate sleeves that mostly delivered sector beta.

| Live sleeve | Mode | Notes |
|-------------|------|--------|
| Innovation Fund | Rule IPS + decision engine | Active |
| Momentum Growth | Rule IPS + decision engine | Active |
| AI Defense & Autonomous | Rule IPS + decision engine | Active |
| LLM-Commando | Dialectic + radar + watcher | Renamed display path **LLM-Commando** (portfolio_id=10); script filenames still `llm_comando_*` for path safety |

**Inactive (DB retained, `is_active=0`):** Value Picks, Growth Value, Income Dividends, Nuclear Renaissance, LLM-ETF Focus, Treasury Reserve. Killed books were flattened at/near market close where possible; residual paper positions that Alpaca marks **inactive/untradable** (e.g. CPRX) may remain on the broker account only — BigClaw DB is cleared for those names; guards treat Alpaca orphans as **warn-only**, not a global halt.

**Website / charts:** performance and holdings views filter `is_active=1` only (`generate_chart.py`, dashboard straighten). See `data/FOUR_SLEEVE_README.txt` on the Pi.
...
### 6.5b Entry Gate — Anti-Extension / Overbought (`autonomous_trader.passes_entry_gate`, July 17 2026)

A mean-reversion / anti-chasing filter applied to **candidates only** (it never force-sells an existing holding) inside `plan_portfolio`, right after the style gate. It is naturally scoped to the 7 rule-based portfolios — the 2 LLM portfolios never reach `plan_portfolio`, and an unknown portfolio name has no gate and always passes.

**Basis.** An evidence-based diagnostic of 415 real buys (all portfolios, 2026-04 → 07) ranked every at-entry feature by correlation with realized alpha vs SPY. The strongest predictor was **extension above the 200-day MA (corr −0.30)**, and the whole top of the list — 5-day momentum, 60-day momentum, volatility, extension vs the 20/50-day MAs, entry-day chase — formed one coherent theme (all negative): the further a name had already run at entry, the worse it did. Options-flow signals ranked dead last (noise). A simulated `ext<+10%` gate flipped the 415 buys from **−1.4% to +1.1%** average return and lifted the win rate from 47% to 59%.

**Style-aware thresholds** (veto a candidate whose extension or RSI exceeds its style's limit; vetoes only on data that is present, so a missing indicator never blocks a buy):

| Portfolio | max % above 200d MA | max RSI(14) |
|-----------|---------------------|-------------|
| Value Picks, Income Dividends | 12% | 70 |
| Growth Value | 15% | 72 |
| Innovation, Momentum, Nuclear, AI Defense | 25% | 80 |

Value/mean-reversion styles gate tight; momentum/growth styles gate loose so only egregious chases are killed (e.g. MPC at +40% over its 200d MA / RSI 88) without gutting a momentum mandate. Data source: `decision_engine.score_ticker` now emits raw `rsi14` + `pct_above_200ma` per ticker, carried through `format_json` across the subprocess JSON boundary into the trader's `signal_map`. Thresholds live in the `ENTRY_GATE` map in `autonomous_trader.py`. The gate is shared by the live trader and the dashboard planned-actions panel (both call `plan_portfolio`).

**Why entry, not exit.** A companion audit of 60 real trailing-stop sells found the absolute stops *protect* the rule-based books (stopped-out names fell a further ~4.65% to date on average), so the stops were **kept**, not removed; a trailing-stop sweep further showed a stop *added on top of good entries makes them worse*. The root cause of underperformance is the entry (chasing extension), and best-in-class opportunity-cost rotation already handles the exit (§7). The gate fixes the entry; the exit was left alone.
...
**Prices moved from yfinance to Alpaca.** A new `get_daily_bars(tickers, start, end)` in `src/alpaca_data.py` fetches daily split/dividend-adjusted OHLCV bars from Alpaca and returns a DataFrame shaped exactly like `yf.download()` (MultiIndex columns `Field x Ticker`), so it is a drop-in for the engine's bulk price fetch. 554 tickers fetch in ~70s with no rate-limit, versus yfinance throttling hard (1811 "Too Many Requests" in one morning, even on SPY/QQQ) and the bulk download timing out. Symbol quirks (`BRK-B` -> `BRK.B`) and ADRs are handled. There are two fallbacks: any individual ticker Alpaca cannot serve falls back to yfinance inside `get_daily_bars`, and if Alpaca returns nothing at all the engine falls back to the original yfinance bulk download. SPY history (used for the relative-strength signal) is now fetched **once** per rescreen and passed into `analyze_falling_knife`, instead of being re-fetched per-ticker (543x). The yfinance earnings-calendar lookup was made fail-fast (`attempts=1`) so a yfinance throttle can no longer block the rescreen. **Data-stream pre-flight (July 3 2026).** yfinance is intermittently unreliable on BULK multi-symbol downloads (a single-symbol fetch can succeed while the ~30-symbol x 1y bulk call returns all-NaN). On July 3 the bulk call NaN-d and both LLM portfolios ran a cycle on a BLANK market snapshot (get_market_snapshot still uses yf.download with no fallback; the decision engine was migrated to Alpaca but the LLM books were not). scripts/data_stream_preflight.py (cron 8:15 CT weekdays, before the 9:00 refresh and 10:00 trade) checks the four streams trading depends on - Alpaca trading API, Alpaca market data (get_daily_bars), Alpaca news, and a REALISTIC bulk yfinance download - and posts a GREEN/CAUTION/RED light to Slack + writes data/data_preflight_status.json. It DETECTS a dead feed at the door; the paired CURE (still pending) is to point the LLM get_market_snapshot at Alpaca get_daily_bars with a yfinance fallback + a blind-data guard so a cycle never trades on a blank snapshot. **Alpaca paper-account reset + pre-trade guard (July 7 2026).** Alpaca silently DELETED equity positions from many paper accounts (confirmed widespread on their community forum) with no orders, fills, journals, or account-activities. BigClaw lost ~$773K of longs; equity went ~$1.02M -> ~$250K with the market flat and only ~$66K traded (impossible from trading = an account-level wipe). Signature: equity fell by exactly the positions' value with NO cash proceeds. Because the DB still listed the positions, the stop/exit sells filled into the emptied account and opened 6 unintended shorts (clamp_sell_to_long did not catch them on all paths). Alpaca RESTORED the account hours later (reconciled 63/63 tickers, shorts unwound). BigClaw's research was never at risk - the journals/outcomes/patterns live in the DB, not the broker account (the reason a broker wipe is survivable and a platform migration is bounded). FIX (commit 0af712eac): `verify_account_synced()` in autonomous_trader.py - before any trading, compare live Alpaca positions to the DB; if >=5 DB-held tickers are missing at Alpaca, set the global MISMATCH kill-switch (logs/ALPACA_MISMATCH.flag) + Slack alert and halt, so a future reset auto-freezes BigClaw instead of trading into a phantom book. Wired into ALL 9 order-placing scripts: autonomous_trader, stop_check, trade_executor, and the 3 Commando + 3 ETF-Focus (cycle/watcher/reconciler) - stop_check and trade_executor had never checked the kill-switch before. Tolerates small fill lag; manual clear to resume; fails open on a transient check error.

**Atomic dashboard-JSON writes (July 1 2026).** `docs/data/signals.json` was corrupted into a torn file — a complete JSON object followed by ~107 KB of leftover bytes from an older, longer version — which crashed `price_refresh` with `JSONDecodeError: Extra data` and failed the whole refresh. Root cause: `signals.json` has FIVE writers (`export_signals.py` ×2 — rescreen output + holdings enrich; `price_refresh.py`; `build_planned_actions.py`; `export_dashboard.py`) and they are not serialized. The 10:00 `autonomous_trader` post-trade dashboard export overlapped `refresh_all`'s rescreen, and two plain `open("w")` writes interleaved into one file. Fix: every signals.json write is now ATOMIC — write to `signals.json.tmp.<pid>` then `os.replace()` (atomic on the same filesystem), so a reader always sees a complete file and concurrent writers can at worst last-writer-win (self-heals next cycle), never tear. The corrupt file was repaired by truncating to the valid object (backed up as `.corrupt-*`). (`portfolios.json`/`market.json` have the same multi-writer shape and could get the same treatment if they ever tear.) **Schedule stagger (July 1 2026).** The underlying overlap was that `refresh_all.sh` and `autonomous_trader.py` BOTH fired at 10:00 CT. The pre-trade refresh was moved to 9:40 CT (system crontab) so the full ~15-min pipeline completes (~9:55) before the 10:00 trader reads signals.json — the trader now acts on fresh, complete scores with no write overlap. The trader only needs the export_signals step (first ~4 min), which finishes ~9:44 regardless. Full refresh_all schedule: 9:00, 9:40, 12:00, 14:00, 16:30 CT; trader 10:00. **Intraday price-only refresh (July 9 2026).** To keep the website's prices and portfolio values fresh between the 5 heavy `refresh_all` anchors, standalone `price_refresh.py` (LLM-free, ~1 min, self-commits/pushes `portfolios.json`/`market.json`/`signals.json`) also runs at 10:15, 10:45, 11:15, 11:45, 12:45, 13:15, 13:45, 14:30 CT — scheduled clear of the `refresh_all` git windows to avoid concurrent-push contention. Net dashboard price freshness is ~30 min through the session; charts and the full dashboard export still refresh only at the 5 anchors. Two further intraday publishes (same day): each intraday slot also runs `build_planned_actions.py` first, so the forward **planned-actions** list stays current (`price_refresh` preserves the injected key when it rewrites signals.json); and a `push_docs.sh` cron at 9:20 / 11:50 / 14:50 CT publishes the **LLM Commando/ETF dashboard panels** right after each dialectic cycle. `push_docs.sh` is an flock-serialized, retrying commit/push helper, so these extra pushers never collide with `refresh_all` / `price_refresh` on the git index lock (they queue rather than fail). **Cash Remaining fix (July 13 2026):** price_refresh.py omitted the authoritative current-cash field from portfolios.json, so the frontend fell back to startingCash-minus-invested (which ignores realized P&L) and showed a portfolio down on the year as still holding its full starting cash. price_refresh now writes cash (=current_cash) and holdingsValue, matching src/export_dashboard.py, so Cash Remaining reflects the DB truth.
...
- **Trading window**: Weekdays, market hours only (Alpaca clock). Rule trader ~10:00 AM–4:00 PM ET; Commando radar/watcher during RTH CT windows.
- **Cash reserve**: Minimum 2% of starting capital retained per portfolio (rule books)
- **Max single order**: $25,000 (rule books)
- **Round-trip limit**: Max 3 round-trips per ticker per week (rule books; prevents churning)
- **Max holdings**: 10 positions per portfolio (rule books)
- **Min holdings**: 7 positions per portfolio (rule books; triggers swap/add when below)
- **Position concentration**: No single position may exceed 20% of total portfolio value (rule books)
- **Long-only**: unexpected Alpaca shorts → global mismatch halt
- **Fresh holdings at LLM sell**: re-read DB at execute; clamp sells to held shares (June 15 short incident)
- **Active-book Alpaca sync**: pre-trade + post-trade verify; self-heal stale flags (July 22)
...
**Invariant 2 — Active-DB shares match Alpaca shares (per ticker)**
```
Σ(holdings.shares for is_active=1 portfolios) for ticker T ≈ Alpaca position qty for T
```
Tolerance: material share drift on **active** books only. Failure means a buy/sell did not record the right qty (partial-fill bug, lost write, or double-write).

**Alpaca orphans (July 22 2026):** positions at Alpaca with **no active-DB row** (e.g. leftover CPRX after a sleeve flatten when the asset is broker-inactive and cannot be sold) are **WARN only** — they must not freeze the entire trading system. Documented in `data/alpaca_orphans.json` when cleared from DB.
...
```
Tolerance: $5 noise. The Alpaca paper account may have pre-existing setup cash that no portfolio claims. INV3 alone is a warning, not always a hard halt (paper setup cash can fail INV3 definitionally).

`scripts/accounting_audit.py` runs all three daily. Critical fail (INV1 or INV2 on **active** books) writes `logs/ALPACA_MISMATCH.flag` and posts CRITICAL Slack.

**Pre/post-trade Alpaca guard (July 7 2026; hardened July 22 2026).** `verify_account_synced()` in `autonomous_trader.py` runs before order placement on all execute paths (rule trader, Commando, watchers, stop_check, trade_executor). Scope = **active portfolios only**. Halt on: unexpected Alpaca shorts, or many active-DB tickers missing at Alpaca (paper-account wipe signature). **Self-heal:** if the flag exists but halt conditions have cleared, any caller clears the flag (not only the daily autonomous_trader cron). **Post-trade:** `post_trade_verify_or_flag()` re-checks after Commando fills. Path constant: `~/bigclaw-ai/logs/ALPACA_MISMATCH.flag` only (do not invent alternate paths).
...
### 9.4 System Crontab Jobs (selected; see live `crontab -l` on Pi)

| Schedule (CT) | Script | Purpose |
|---------------|--------|---------|
| `0 9, 11:30, 14:30` weekdays | `llm_comando.py --cycle …` | Commando full dialectic sessions |
| `*/5 8-15` weekdays | `llm_comando_watcher.py` | Intraday triggers + mechanical exits |
| `*/2 8-15` weekdays | `llm_comando_radar.py` | Event radar + price_is_news |
| `50 14` weekdays | `llm_comando_reconciler.py` | End-of-day exit backstop |
| every 15 min RTH | `stop_check.py` | Trailing stops (all books) |
| various | rule DE + `autonomous_trader` | Active rule sleeves |

### 9.4b System Crontab Jobs (legacy heading retained)
...
- **Intraday price-only** (~every 30 min, 10:15–14:30 CT) via standalone `price_refresh.py`: prices + portfolio values only, LLM-free, for near-live dashboard freshness between the refresh_all anchors
...
| `llm_comando_portfolio.json` | LLM-Commando daily bull/bear/judge output + trades | 1×/day weekday |
| `llm_comando_journal.jsonl` | LLM-Commando append-only journal | per-cycle |
| `llm_comando_outcomes.jsonl` | LLM-Commando trade closure log | continuous |
| `llm_comando_pending_triggers.json` | LLM-Commando intraday triggers + fires counter | per-cycle, per-fire |
