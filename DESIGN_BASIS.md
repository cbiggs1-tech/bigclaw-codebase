...
**Curated LESSONS block + extension/RSI in the compete view (July 17 2026).** An evidence-based diagnostic of BigClaw's own trades (415 real buys, 2026-04 → 07) found the single strongest predictor of poor forward return is how *extended* a name is at entry — buys far above their 200-day MA or overbought (RSI>70) mean-revert, the most-extended quintile returning ~−8% vs SPY. A shared `scripts/llm_lessons.py` module now injects a curated **LESSONS LEARNED** block into both LLM portfolios' data feed every cycle (Bull, Bear, and Judge all read it): four durable, evidence-backed principles — (1) extension is risk, not confirmation; (2) buying strength is not an edge; (3) opportunity cost is the exit — rotate the weakest hold into a materially better name (thesis_changed), absolute stops added nothing once entries were disciplined; (4) a live catalyst is the edge, price alone is a bandwagon. These are **information, not rules** — the dialectic still decides everything, preserving the portfolios as autonomous controls, and the existing recursive journal loop extends them with each book's own outcomes. Comando additionally computes 14-day RSI and %-above-200-day-MA per candidate/holding (snapshot history extended 3mo→1y) and flags overbought/extended names in the CANDIDATE STRENGTH RANKING with a `⚠extended` marker so the lesson is actionable. This is a deliberate **soft-vs-hard governance experiment**: the same finding is a hard deterministic veto for the 7 rule-based portfolios (§6.5b) and soft LLM-weighed information here — testing whether a rule or LLM judgment handles extension better. (Interpreter note: the LLM books run under `venv/bin/python`, which carries slack_sdk/anthropic/numpy; the rule-based engine runs under system `python3`, which carries `ta`.)
