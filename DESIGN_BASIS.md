...
| Version | June 18, 2026 (current through June 18 2026, 15:30 CT) |
| Author | Curtis Biggs |
| Hostname | BigClaw |
...
**Macro-regime cycle pack (added June 16 2026).** `get_market_snapshot` now also pulls ^VIX, HYG, LQD, ^TNX, and IWM (1-year history) and computes 3-month (63d) and 6-month (126d) returns alongside 1d/5d/30d. A "MACRO REGIME (cycle tells)" block renders into both portfolios' context: VIX level + trend, high-yield-vs-investment-grade credit (HYG vs LQD), the 10-year yield, offense/defense (XLY vs XLP), breadth (IWM vs SPY), and 3-month sector-rotation leaders/laggards. The two portfolios consume it differently. **ETF Focus** runs a "CYCLE POSITIONING" mandate — call the cycle phase from the regime block and position sectors months-out (lead the rotation; do not chase what already ran), the forward edge a months-horizon book needs instead of lagging news. **Comando** runs a "VOLATILITY REGIME" mandate — use VIX to set aggression: low/falling VIX in an uptrend means press and rotate through more leaders with modest sizing; high/spiking VIX means throttle back, size down, favor cash. All free via yfinance (no FRED key).

**Comando momentum sourcing (added June 17 2026).** Comando previously saw only news-makers (the top names mentioned in the last 24h) plus held + watchlist — a small, lagging set, which is why gap-analysis (correctly rejecting priced-in headline names) often left it sitting in cash. `discover_momentum_leaders()` adds a forward "what is trending" view: it pulls the rule-based engine's daily screened-and-scored universe (`docs/data/signals.json`, ~537 names), ranks the top-by-score on momentum (1-month + 3-month relative strength + proximity to the 6-month high) via one batch yfinance download, and renders the top leaders as a "MOMENTUM LEADERS" block. The Judge's MOMENTUM SOURCING mandate now weighs two views — news-makers (often priced, run through gap-analysis) and momentum leaders (trending) — and in a confirmed bull regime (low VIX + healthy breadth, read off the macro-regime block) treats a clean, not-yet-parabolic uptrend as a valid thesis even without a headline: be active, many small shots. Comando-only — ETF Focus sources sectors via the cycle pack.
