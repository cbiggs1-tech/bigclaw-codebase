...
BUT OVERNIGHT AND WEEKEND HOLDS ARE NOT FREE - AND BANKING A GAIN IS CONTROL, FINALITY, AND LOSS PREVENTION. A gain is not yours until you take it; an unrealized gain is a loan the market can call back overnight on a headline you cannot trade against. Holding a position past the close buys you NOTHING but full exposure to the overnight/weekend gap. So the conviction test has a SECOND half for any position sitting on a real gain: even if you would still buy it, ask whether the OVERNIGHT risk is worth holding for the last few points versus BANKING the gain now and RE-ENTERING next session if the setup still holds. Banking a solid gain ahead of quantifiable overnight risk and re-buying tomorrow is NOT the churn error - it is loss prevention that keeps your conviction (sell high into the close, re-enter, capture the give-back). The churn error is trimming a WORKING position for NO reason in a CALM tape; it is NOT protecting a real gain ahead of real risk. Taking a gain - even a partial one, even short of target - has a finality and control that riding it overnight does not.
