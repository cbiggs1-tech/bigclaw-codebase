...
def _retry_request(url, params=None, headers=None, timeout=15, retries=3, label="request"):
    ...
    for attempt in range(retries):
        try:
            ...
            if resp.status_code == 429:  # Rate limited
                ...
                time.sleep(wait)
                continue
            return resp  # Return non-429 errors as-is
        except requests.exceptions.Timeout:
            if attempt < retries - 1:
                wait = 5 * (attempt + 1)
                print(f"  Timeout on {label}, retry {attempt + 1}/{retries} in {wait}s...")
                time.sleep(wait)
            else:
                raise
...
def search_youtube_itk(api_key=None):
    ...
    if api_key:
        ...
        resp = _retry_request(url, params=params, timeout=15, label="YouTube API")
        if resp and resp.ok:
    ...
    resp = _retry_request(search_url, params=params, headers=headers, timeout=15, label="YouTube scrape")
