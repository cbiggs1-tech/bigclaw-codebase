...
# ---------------------------------------------------------------------------
# Style-specific signal weights per portfolio
# Each style multiplies signal categories differently.
# Weight 0 = ignore, 1 = normal, 2 = double emphasis
# Signal categories: RSI, MACD, SMA50, SMA200, Cross, RelStrength (technical)
#                    EarningsGrowth, RevenueGrowth, PE, DebtEquity (fundamental)
...
def load_portfolios(filter_name=None):
    ...
    # LEFT JOIN so portfolios with zero holdings still appear
    q = """SELECT p.id, p.name, p.current_cash, h.ticker, h.shares, h.avg_cost
           FROM portfolios p LEFT JOIN holdings h ON p.id = h.portfolio_id AND h.shares > 0
           WHERE p.is_active = 1"""
    if filter_name:
        q += " AND p.name = ?"
...
def analyze_technicals(close):
    ...
    if macd_prev < signal_prev and macd_line > signal_line:
        ...
        signals.append(("MACD", -0.5, "MACD bearish crossover"))
...
def analyze_quality_fundamentals(info):
    """Return quality/style-gate fundamental signals: PEG, ROE, FCF, GrossMargin, PayoutSafety.

    These complement analyze_fundamentals() — that function handles earnings growth,
    revenue growth, P/E, debt, short interest, and dividends. These handle the
    deeper metrics that differentiate portfolio styles (Buffett, Lynch, Income, etc.).
    """
    signals = []
    ...
    if roe is not None:
        if roe >= 0.20:
            ...
            signals.append(("ROE", 1, f"ROE {roe:.0%} (good)"))
        elif roe >= 0.10:
            signals.append(("ROE", 0, f"ROE {roe:.0%} (acceptable)"))
        elif roe > 0:
            signals.append(("ROE", -1, f"ROE {roe:.0%} (weak)"))
        else:
        ...
        if fcf > 0:
            ...
            if mcap and mcap > 0:
                ...
                if fcf_yield >= 0.05:
                    ...
                    signals.append(("FCF", 1, f"FCF positive (${fcf/1e9:.1f}B)"))
            else:
                signals.append(("FCF", 1, f"FCF positive"))
        else:
    ...
    if gm is not None:
        if gm >= 0.40:
            ...
            signals.append(("GrossMargin", 0, f"gross margin {gm:.0%} (decent)"))
        else:
    ...
    if payout is not None and div_yield and div_yield > 0:
        if payout < 0.60:
            ...
            signals.append(("PayoutSafety", 0, f"payout ratio {payout:.0%} (moderate)"))
        else:
            signals.append(("PayoutSafety", -1, f"payout ratio {payout:.0%} (stretched)"))
    elif div_yield and div_yield > 0 and payout is None:
        # Pays dividend but no payout data — neutral
        signals.append(("PayoutSafety", 0, "payout ratio unavailable"))
...
def analyze_relative_strength(ticker, ticker_data, market_data):
    ...
    if diff > 0.02:
        ...
        signals.append(("RelStrength", -0.5, f"underperforming {etf} by {abs(diff):.1%}"))
    else:
...
def fetch_bond_score():
    ...
    try:
        ...
        logger.warning(f"Bond signals error: {e}")
...
def score_ticker(ticker, market_data, prices, bond_combined, bond_weight, style_weights=None):
    """Score a single ticker with style-specific weights.

    style_weights: dict mapping signal category name to multiplier (0-2).
    If None, all signals weighted equally (backward compatible).
    """
    ...
    # Apply style weights to get weighted score
    base_score = 0
    for cat, raw, desc in all_signals:
    ...
    if vo_score > 0:
        ...
        base_score += vo_score * vo_weight

    # Round to integer score
    ...
    if round(sum(s[1] for s in all_signals if s[0] != "ValueOverride")) <= -5 and vo_score > 0 and vo_score < 3:
        score = round(sum(s[1] * style_weights.get(s[0], 1.0) for s in all_signals))
        if score > -3:
            score = -3
...
def build_portfolio_optimization(portfolios, results_by_ticker, market_data, candidate_scores=None):
    ...
    for pname, pdata in portfolios.items():
        ...
        # SGOV is money market — exempt from position count and optimization
        non_sgov_holdings = [h for h in holdings if h["ticker"] != "SGOV"]
        num_holdings = len(non_sgov_holdings)

        # Rank holdings by score (exclude SGOV)
        ranked = []
        for h in non_sgov_holdings:
            ticker = h["ticker"]
...
def run_analysis(portfolio_filter=None, rescreen=False):
    ...
    if rescreen:
        ...
        for pname, udata in universes.items():
            # udata is {"holdings": [...], "candidates": [...]}
            all_universe = udata.get("holdings", []) + udata.get("candidates", [])
    ...
    # Fetch bond signals
    logger.info("Fetching bond market signals...")
    bond_data = fetch_bond_score()
    bond_combined = bond_data.get("combined_score", 0)

    # Build ticker->portfolio mapping for bond weight + style lookup
    ticker_portfolios = {}
    for pname, pdata in portfolios.items():
        for h in pdata["holdings"]:
    ...
    # Also map candidate tickers to their portfolios for style scoring
    if rescreen and universes:
        for pname, udata in universes.items():
            for t in udata.get("holdings", []) + udata.get("candidates", []):
                if t not in ticker_portfolios:
                    ticker_portfolios[t] = []
                if pname not in ticker_portfolios[t]:
                    ticker_portfolios[t].append(pname)

    # Per-ticker analysis with style-specific scoring
    results = []
    results_by_ticker = {}  # ticker -> best result (highest score across portfolios)
    ...
    results = [r for r in results if r["score"] >= 0 or r["ticker"] in held_tickers]
    ...
    if rescreen and universes:
        for pname, udata in universes.items():
            ...
            scored = []
            for t in all_universe:
                # Use per-portfolio score if available, fall back to best score
                r = results_by_portfolio.get((pname, t)) or results_by_ticker.get(t)
                if r:
    ...
    # Merge candidate scores into signals so autonomous trader can act on them
    existing_tickers = {r["ticker"] for r in results}
    for r in results_by_ticker.values():
        if r["ticker"] not in existing_tickers:
            results.append(r)

    # Build per-portfolio signal map: {portfolio_name: {ticker: {score, label}}}
    portfolio_signals = {}
    for (pname, ticker), r in results_by_portfolio.items():
        if pname not in portfolio_signals:
            portfolio_signals[pname] = {}
        portfolio_signals[pname][ticker] = {"score": r["score"], "label": r["label"]}
